...
optimizers={'sgd':SGD,'momentum':Momentum,'rmsprop':RMSprop,'nag':NAG}
...
def compute_f1_score(y_true,y_pred,num_classes=10):
    ...
    if len(y_true.shape)>1 and y_true.shape[1]>1: #gotta convert to class indices if one-hot encoded
    ...
    for class_idx in range(num_classes):
        tp=np.sum((y_true==class_idx)&(y_pred==class_idx)) #calc true positives, false positives, false negatives
    ...
    return np.mean(f1_scores) #returning macro f1
...
def train_with_wandb(config=None,project='da6401-assignment1_try2',entity=None):
    with wandb.init(config=config,project=project,entity=entity):
        ...
        def enhanced_epoch_callback(metrics):
            ...
            wandb.log({'epoch':epoch,'train_loss':metrics.get('train_loss',0),'train_acc':metrics.get('train_acc',0),'train_f1':metrics.get('train_f1',0),'val_loss':metrics.get('vapython inference.py -d fashion_mnist --model_path models/fashion_mnist/best_model.npy --config_path models/fashion_mnist/config.jsonl_loss',0),'val_acc':metrics.get('val_acc',0),'val_f1':metrics.get('val_f1',0)})
            ...
            wandb.log({'overfitting_gap':metrics.get('train_loss',0)-metrics.get('val_loss',0),'accuracy_gap':metrics.get('train_acc',0)-metrics.get('val_acc',0),'f1_gap':metrics.get('train_f1',0)-metrics.get('val_f1',0),'best_val_acc_so_far':best_val_acc,'best_val_loss_so_far':best_val_loss,'best_val_f1_so_far':best_val_f1})
        ...
        model.train(
            ...
            xval=X_val,yval=y_val_oh,
            callback=enhanced_epoch_callback
        )

        test_loss,test_acc,test_f1=model.evaluate(X_test,y_test_oh)
        
        final_metrics={'final_test_loss':test_loss,'final_test_acc':test_acc,'final_test_f1':test_f1,'final_train_loss':train_losses[-1] if train_losses else 0,'final_train_acc':train_accs[-1] if train_accs else 0,'final_train_f1':train_f1s[-1] if train_f1s else 0,'final_val_loss':val_losses[-1] if val_losses else 0,'final_val_acc':val_accs[-1] if val_accs else 0,'final_val_f1':val_f1s[-1] if val_f1s else 0,'best_val_acc':best_val_acc,'best_val_loss':best_val_loss,'best_val_f1':best_val_f1,'convergence_epoch':convergence_epoch,'final_overfitting_gap':train_losses[-1]-val_losses[-1] if train_losses and val_losses else 0}
        wandb.log(final_metrics)
        
        dataset_folder=f'models/{cfg.dataset}' #making dataset specific folder
        ...
        if best_val_f1>current_best_val_f1: #saving model based on validation f1
            with open(best_f1_file,'w') as f:
                f.write(str(best_val_f1))
            
            best_weights=model.get_weights() #saving model weights using get_weights()
            np.save(best_model_path,best_weights)
            
            config={'dataset':cfg.dataset,'input_size':X_train.shape[1],'hidden_sizes':hidden_sizes,'output_size':10,'activation':cfg.activation,'weight_init':cfg.weight_init,'loss':cfg.loss,'optimizer':cfg.optimizer,'learning_rate':cfg.learning_rate,'batch_size':cfg.batch_size,'epochs':cfg.epochs,'weight_decay':cfg.weight_decay,'best_test_f1':float(test_f1),'best_val_f1':float(best_val_f1),'best_val_acc':float(best_val_acc),'convergence_epoch':convergence_epoch,'wandb_run_id':wandb.run.id} #saving model config
            with open(best_config_path,'w') as f:
                json.dump(config,f,indent=4)
            
            print(f"\n new best Val F1: {best_val_f1} (prev: {current_best_val_f1})\nCorresponding Test F1: {test_f1}")
            ...
            print(f"\ncurrent best Val f1 remains: {current_best_val_f1} (this run: {best_val_f1})")
...
def main():
    ...
    if args.sweep:
        sweep_config={'method':'random','metric':{'name':'final_val_f1','goal':'maximize'},
                      'parameters':{'dataset':{'value':args.dataset},
                                'epochs':{'values':[10,5,3,7]},
                                'batch_size':{'values':[16,32,64]},
                                'learning_rate':{'values':[0.001,0.005,0.01,0.05]},
                                'optimizer':{'values':['sgd','momentum','rmsprop','nag']},
                                'hidden_size':{'values':[[128,64,32],[128,128,64],[128,64,32,16],[128,128,128]]},
                                'num_layers':{'value':2},#this gets over write by len hiddenn size
                                'activation':{'values':['relu','sigmoid','tanh']},
                                'loss':{'values':['cross_entropy','mse']},
                                'weight_init':{'values':['xavier','random']},
                                'weight_decay':{'value':args.weight_decay}}} #doing hyperparameter sweep
        ...
        class Config: #running single training with command line args
            pass
